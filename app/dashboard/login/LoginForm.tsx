  ...
  return (
    <main style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'var(--color-canvas)' }}>
      <form onSubmit={handleLogin} style={{ width:'100%', maxWidth:400, padding:'2rem' }}>
        ...
        <label htmlFor="email" style={{ display:'block', fontSize:'var(--text-sm)', marginBottom:'0.25rem' }}>
        ...
        />
        <label htmlFor="password" style={{ display:'block', fontSize:'var(--text-sm)', marginBottom:'0.25rem' }}>
          Password <span aria-hidden="true" style={{ color: '#B91C1C' }}>*</span>
        </label>
        <input
        ...
        <button
          ...
          disabled={loading}
          aria-busy={loading}
          style={{ width:'100%', padding:'0.75rem', background:'var(--color-ink)', color:'var(--color-canvas)', border:'none', borderRadius:4, cursor: loading ? 'wait' : 'pointer', fontFamily:'var(--font-body)', opacity: loading ? 0.7 : 1, display: 'inline-flex', alignItems: 'center', gap: '8px', justifyContent: 'center' }}
        >
          {loading ? (
            <>
